 ...
 * - .env yüklenir
 * - Config set edilir
 * - Oturum & güvenlik politikaları
 * - Global hata yakalayıcı
 * - Router + Middleware (Auth, Role, CSRF, Guest)
 * - DEBUG overlay
 */

///////////////////////////////////////////////////////////////
// 0) Boot ölçümleri (guard ile)
///////////////////////////////////////////////////////////////
if (!defined('SYSTEM_START_TIME'))   define('SYSTEM_START_TIME', microtime(true));
if (!defined('SYSTEM_START_MEMORY')) define('SYSTEM_START_MEMORY', memory_get_usage());
...
///////////////////////////////////////////////////////////////
// 2) Composer + Autoloader + Security helpers
///////////////////////////////////////////////////////////////
...
// Fonksiyon dosyaları autoloadera girmez, elle dahil et.
...
if (file_exists($envFile)) {
    foreach (preg_split('/\R/', file_get_contents($envFile)) as $line) {
        $line = trim($line);
...
///////////////////////////////////////////////////////////////
// 5) Session (yalnızca aktif DEĞİLSE ayarla)
///////////////////////////////////////////////////////////////
...
try {
    ...
    // AUTH (Yönetici)
    // --------------------------
    $router->get('',        TN_AuthController::class, 'showLogin', [fn() => TN_AuthMiddleware::requireGuest()]);
    $router->get('login',   TN_AuthController::class, 'showLogin', [fn() => TN_AuthMiddleware::requireGuest()]);
    $router->post('login',  TN_AuthController::class, 'login',     [fn() => TN_AuthMiddleware::requireCsrf()]);
    $router->get('logout',  TN_AuthController::class, 'logout',    [TN_AuthMiddleware::class]);

    // alias’lar
    $router->get('auth',        TN_AuthController::class, 'showLogin', [fn() => TN_AuthMiddleware::requireGuest()]);
    $router->get('auth/login',  TN_AuthController::class, 'showLogin', [fn() => TN_AuthMiddleware::requireGuest()]);
    $router->post('auth/login', TN_AuthController::class, 'login',     [fn() => TN_AuthMiddleware::requireCsrf()]);
    $router->get('auth/logout', TN_AuthController::class, 'logout',    [TN_AuthMiddleware::class]);

    // --------------------------
    // AUTH (Öğrenci)
    // --------------------------
    $router->get('student-login',        TN_StudentAuthController::class, 'showLogin', [fn() => TN_AuthMiddleware::requireGuest()]);
    $router->post('student-login',       TN_StudentAuthController::class, 'login',     [fn() => TN_AuthMiddleware::requireCsrf()]);
    $router->get('auth/student-login',   TN_StudentAuthController::class, 'showLogin', [fn() => TN_AuthMiddleware::requireGuest()]);
    $router->post('auth/student-login',  TN_StudentAuthController::class, 'login',     [fn() => TN_AuthMiddleware::requireCsrf()]);
